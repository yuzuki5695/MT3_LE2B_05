...
void RenderScene(const Sphere spheres[], const Vector3 translates[], const Vector3 rotates[], const Vector3 scales[], const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewportMatrix, const uint32_t colors[]) {
	Matrix4x4 worldMatrices[3];
	for (int i = 0; i < 3; ++i) {
		Matrix4x4 scaleMatrix = MakeScaleMatrix(scales[i]);
		Matrix4x4 rotationMatrix = MakeRotationMatrix(rotates[i]);
		Matrix4x4 translationMatrix = MakeTranslationMatrix(translates[i]);
		Matrix4x4 localMatrix = Multiply(translationMatrix, Multiply(rotationMatrix, scaleMatrix));
		if (i > 0) {
			worldMatrices[i] = Multiply(worldMatrices[i - 1], localMatrix);
		} else {
	...
	// 各球体を描画
	for (int i = 0; i < 3; ++i) {
		// ワールド行列をビュー・プロジェクション・ビューポート行列に適用
		//Matrix4x4 worldViewProjectionMatrix = Multiply(worldMatrices[i], viewProjectionMatrix);
		//Matrix4x4 finalMatrix = Multiply(worldViewProjectionMatrix, viewportMatrix);

		// 球体を描画
		DrawSphere(spheres[i], viewProjectionMatrix, viewportMatrix, colors[i]);
	}

	Vector3 a = Transform(Vector3(translates[0].x, translates[0].y, translates[0].z),Multiply(worldMatrices[0], Multiply(viewProjectionMatrix, viewportMatrix)));
	Vector3 b = Transform(Vector3(translates[1].x, translates[1].y, translates[1].z), Multiply(worldMatrices[1], Multiply(viewProjectionMatrix, viewportMatrix)));
	Vector3 c = Transform(Vector3(translates[2].x, translates[2].y, translates[2].z), Multiply(worldMatrices[2], Multiply(viewProjectionMatrix, viewportMatrix)));

	// 線を描画（球体AとBをつなぐ線、球体BとCをつなぐ線）
	Novice::DrawLine(int(a.x), int(a.x), int(b.x), int(b.x),0xFFFFFFFF);
	Novice::DrawLine(int(b.x), int(b.x), int(c.x), int(c.x),0xFFFFFFFF);
}
